...
gulp.task('sprite', function () {
    return gulp.src(config.src + '/icons/*.png')//需要合并的图片地址
        .pipe(spritesmith({
            imgName: config.src + 'Content/Company2/icons/sprite.png',//保存合并后图片的地址
            cssName: config.src + 'Content/Company2/css/sprite.css',//保存合并后对于css样式的地址
            padding:5,//合并时两个图片的间距
            algorithm: 'binary-tree',//注释1
            cssTemplate:config.src + "/libs/sprite.txt"//注释2
        }))
        .pipe(gulp.dest('dist/'));
});
